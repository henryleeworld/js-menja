...
function renderScoreHud() {
    if (isCasualGame()) {
        ...
        scoreNode.innerText = `分數：${state.game.score}`;
        scoreNode.style.display = 'block';
        cubeCountNode.style.opacity = 0.65;
    }
    cubeCountNode.innerText = `切塊數：${state.game.cubeCount}`;
}
...
function renderMenus() {
    ...
    switch (state.menus.active) {
        ...
        case MENU_SCORE:
            ...
            if (isNewHighScore()) {
                highScoreLblNode.textContent = '新高分！';
            } else {
                highScoreLblNode.textContent = `高分：${formatNumber(getHighScore())}`;
            }
